!function (a, b, c) {
	...
	d.prototype = {
		_init: function () {
			...
			m = l.length;
			if (0 === m)
				return alert("Missing icons config!"), !1;
				...
				for (var k = 0; i > k; k++)
					if (a = h[k].path, b = h[k].file || ".jpg", c = h[k].placeholder, d = h[k].alias, a)
						if (d) {
							...
							e = c.replace(new RegExp("{alias}", "gi"), "(\\d+?)"), g.$content.html(g.$content.html().replace(new RegExp(e, "gm"), '<img class="emoji_icon" src="' + a + "$1" + b + '"/>'));
					else
...
function emojisParse (sourceText, icons) {
	var num = icons.length;
	if (num > 0) {
		for (var i = 0; i < num; i++) {
			var path = icons[i].path;
			var file = icons[i].file || ".jpg";
			var placeholder = icons[i].placeholder;
			var alias = icons[i].alias;
			var newAlias = {};
			if (path) {
				if (alias) {
					for (var alia in alias)
						alias.hasOwnProperty(alia) && (newAlias[alias[alia]] = alia);
					
					var e = placeholder.replace(new RegExp("{alias}", "gi"), "([\\s\\S]+?)");
					var f = new RegExp(e, "gm");
					return sourceText.replace(f, function (placeholder, alias) {
						var e = newAlias[alias];
						return e ? '<img class="emoji_icon" src="' + path + e + file + '"/>' : placeholder;
					})
				} else {
					return sourceText;
				}
